...
| Session rules copied verbatim from the reference platform | 30-min inactivity, no midnight reset, no campaign split, `session_id` = unix seconds at session start. Keeps minuano's derived sessions comparable to the reference platform's. | Inventing our own rollover rule: no upside, breaks comparability. |
